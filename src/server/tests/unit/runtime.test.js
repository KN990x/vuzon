...
test('parseCookieSecure: sin valor → false (HTTP homelab)', () => {
...
test('parseTrustProxy: palabras clave de Express que antes caían a false', () => {
  assert.equal(parseTrustProxy('loopback'), 'loopback');
  assert.equal(parseTrustProxy('LinkLocal'), 'linklocal');
  assert.equal(parseTrustProxy('uniquelocal'), 'uniquelocal');
});

test('parseTrustProxy: listas de IP/CIDR se pasan tal cual a Express', () => {
...
test('parseTrustProxy: valor irreconocible cae a false pero avisa', () => {
  const warnings = [];
  const warn = (message) => warnings.push(message);

  assert.equal(parseTrustProxy('sí-por-favor', { warn }), false);
  assert.equal(warnings.length, 1);
  assert.match(warnings[0], /TRUST_PROXY/);
});

test('parseTrustProxy: los valores válidos no generan aviso', () => {
  const warnings = [];
  const warn = (message) => warnings.push(message);
...
test('getServerRuntime incluye trustProxy y cookieSecure', () => {
